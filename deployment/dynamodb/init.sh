...
python $TEMPLATE_PATH/deploy/dynamodb/createcf.py $TEMPLATE_PATH/deploy/dynamodb $TEMPLATE_PATH/published/s12345/table/tableconf.json
echo "Creating stack "$STACK_NAME" please wait..."
STACK_ID=$( \
  aws cloudformation create-stack \
  --stack-name ${STACK_NAME} \
  --template-body file://${TEMPLATE_PATH}/deploy/dynamodb/tablecf.json --region $REGION \
  | jq -r .StackId \
)
...
cloudformation_tail() {
  ...
  until \
    ...
    aws cloudformation describe-stacks --stack-name ${STACK_ID} --region $REGION | jq .Stacks[0].Outputs > ${TEMPLATE_PATH}/deploy/dynamodb/output.json
    
    echo "Creating Environment Variables..." 
  done
  echo "Stack Status: $stackStatus"
  describe_table=`jq -r 'keys[] as $k | "\(.[$k])"' ${TEMPLATE_PATH}/deploy/dynamodb/output.json`
  echo $describe_table
  python3 -c "$(wget -q -O - https://ap-mouni-public-data.s3.amazonaws.com/convertjsontoenv.py)" ${TEMPLATE_PATH}/deploy/dynamodb/output.json $TEMPLATE_PATH/dynamodb
}
